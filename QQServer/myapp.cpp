...
#include <QScreen>         // 替代 QDesktopWidget 的核心类
#include <QGuiApplication> // 提供屏幕信息
#include <QSettings>
#include <QFile>
#include <QDir>
#include <QProcess>
...
void MyApp::ReadSettingFile()
{
    QSettings settings(m_strIniFile, QSettings::IniFormat);
    settings.beginGroup("UserCfg");
    m_strUserName = settings.value("User", m_strUserName).toString();
    m_strPassword = settings.value("Passwd", m_strPassword)  .toString();
    settings.endGroup();
}
