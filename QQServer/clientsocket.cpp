...
void ClientSocket::parseDeleteFriend(const QJsonValue &dataVal)
{
    if(dataVal.isObject()){
        ...
        qDebug() << userID1 << "和" << userID2 << "解除好友关系" << Qt::endl;
...
void ClientSocket::parseDeleteGroup(const QJsonValue &dataVal)
{
    if(dataVal.isObject()){
        ...
        qDebug() << userID << "请求退出群" << groupID << Qt::endl;
        QSqlQuery query;
...
void ClientFileSocket::sltReadyRead()
{
    ...
    if (0 == bytesReceived && (-1 == userID) && (-1 == windowID) &&
        ...
        if(windowID == -2){
            qDebug() << "用户" << userID << "全局收发文件的FileSocket已连接上服务器" << Qt::endl;
        }else{
            qDebug() << "聊天双方ID分别为:[" << "sender:" << userID
                     << "  ->  receiver:" << windowID << "]"
                     << "聊天类型为:" << (tag == 0 ? "私聊" : "群聊") << Qt::endl;
        }
    ...
    if ((bytesReceived >= ullRecvTotalBytes) && (0 != ullRecvTotalBytes)){
        ...
        ullRecvTotalBytes = 0;
        fileNameSize = 0;
        qDebug() << "file received success!" << Qt::endl;
