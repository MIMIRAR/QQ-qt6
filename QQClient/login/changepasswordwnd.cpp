...
void ChangePasswordWndPrivate::sltOKBtnClicked()
{
    ...
    if(id.isEmpty()){
        ...
        int cnt = id.size();
        for(int i = 0;i < cnt;i++){
            if(id[i] >= QChar('0') && id[i] <= QChar('9')){  // 使用 QChar 进行比较
                continue;
            }else{
                this->setFixedSize(430,350);
                m_notifyMsg->move(0,330);
        ...
        if(oldPwd.isEmpty()){
            ...
            if(cnt > 20){
                ...
                for(int i = 0;i < cnt;i++){
                    if( (oldPwd[i] >= QChar('A') && oldPwd[i] <= QChar('Z')) ||  // 使用 QChar 进行比较
                        (oldPwd[i] >= QChar('a') && oldPwd[i] <= QChar('z')) ||  // 使用 QChar 进行比较
                        (oldPwd[i] >= QChar('0') && oldPwd[i] <= QChar('9')) ||  // 使用 QChar 进行比较
                        oldPwd[i] == QChar('_')){  // 使用 QChar 进行比较
                        continue;
                    }else{
                        this->setFixedSize(430,350);
                        m_notifyMsg->move(0,330);
                ...
                if(newPwd.isEmpty()){
                    ...
                    if(cnt > 20){
                        ...
                        for(int i = 0;i < cnt;i++){
                            if( (newPwd[i] >= QChar('A') && newPwd[i] <= QChar('Z')) ||  // 使用 QChar 进行比较
                                (newPwd[i] >= QChar('a') && newPwd[i] <= QChar('z')) ||  // 使用 QChar 进行比较
                                (newPwd[i] >= QChar('0') && newPwd[i] <= QChar('9')) ||  // 使用 QChar 进行比较
                                newPwd[i] == QChar('_')){  // 使用 QChar 进行比较
                                continue;
                            }else{
                                this->setFixedSize(430,350);
                                m_notifyMsg->move(0,330);
